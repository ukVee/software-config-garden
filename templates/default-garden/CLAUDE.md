...
3. **Reserved filenames** (`CLAUDE.md`, `instructions.md`, `notes/`, `last_updated.md`, `refs.md`, `troubleshooting/`) recur with the same meaning everywhere. See `meta/reserved-filenames.md`.
...
| what's installed on the system | `snapshots/packages/<tool>/` | `packages/<tool>/notes/` for quirks |
...
| something that broke and got fixed | `journal/incidents/incident-YYYYMMDD-<slug>.md` | the affected domain's `troubleshooting/` |
...
- **When the user changes something on the system**, update the relevant `notes/` and bump the `Last reviewed:` header on anything that referenced the old state. If a snapshot exists for the changed thing, run its refresh script.
...
1. **Packages** — list installed packages for each package manager; write the snapshot under `snapshots/packages/<tool>/` and quirks under `packages/<tool>/notes/`.
