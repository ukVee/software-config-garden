...
| Cross-device sync pillar (M5) | 🔨 **M5a transport + pairing ✅ COMPLETE (2026-06-09 → 2026-06-10)** — `softfig-net` Noise tunnel + SAS pairing + `peers.toml` ring + mDNS + zero-trust relay; keeperd hosts it, `softfig pair`/`peers`/`unpair` CLI. **No data crosses yet.** M5b (read-only replica) is next + undesigned; M5c (multi-ref union-mount), M5d (shared-subtree keying), M5e (write-turn sync) follow |
...
cargo test --workspace           # 248 tests (M5a-1..3: +52 softfig-net + +6 softfig-vault transport; M5a-4: +2 keeper_toml, +8 m5a4_pairing; M4a: +11 softfig-deploy; M3a/M3b/M-onboard/M2x carryovers), <3s with fast Argon2
...
- **M5a (transport + pairing) shipped** (`softfig-net` + keeperd net host + `softfig pair`/`peers`/`unpair`) — the cross-device *channel + recognition* foundation. Still deferred, each warranting its own decision-file iteration: actual data/VCS sync (M5b replica → M5e write-turn), the unlock trust-matrix / peer-unlock / panic-counter, TPM self-path, replica-only mode. **Pairing ≠ unlock authority ≠ shared-subtree membership** — the M5a `peers.toml` ring is the network layer only; don't conflate it with the unlock-ACL `trust.toml`. Layer B shipped in M2b (whole-file seals) and M2c (inline `<vault id="…">…</vault>` regions); the FUSE plaintext view, the `softfig reveal` flow, and the audit-intent shape are all live.
